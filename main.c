...
int use = 0;
struct regtab{	//¼È¦s¾¹ªí 
    char name[regtabNameMax];
    int code;
};
struct regtab reg[9] = { {"A",0}, {"X",1}, {"L",2}, {"PC",8}, {"SW",9}, {"B",3}, {"S",4}, {"T",5}, {"F",6}, };

struct optab_Node{	//¾Þ§@½Xªí  (¾Þ§@½Xªíªº¸ê®Æ¼g¦bD0746323_¦¿©É§¡_optab.txt¸Ì) 
...
struct symlit_Node{	//²Å¸¹ªí¥H¤Î±`¼Æªí 
...
void regtabPrint(void) {	//¦L¥X¼È¦s¾¹ªí 
...
optab_node * optabNewNode(void) {	//·s¼W¤@µ§¾Þ§@½X 
    optab_node * add = malloc(sizeof(optab_node));
    add->next = NULL;
    return add;
}

int key(char name[opcodeNameMax]) {	//ºâ»®§Çªkªºkey (¥þ³¡ascii½X¬Û¥[¡A¦A%10) 
    int i, sum = 0;
...
void optabPrint(optab_node * head, int hash, int * row) {	//¦L¥X¾Þ§@½Xªí 
    optab_node * ptr;
    if(head->next == NULL) {
        printf("empty\n");
    }
...
void optabCreate(void) {	//§âD0746323_¦¿©É§¡_optab.txt¸Ìªº¾Þ§@½X¥H»®§Çªk§@±Æ¦C¨Ã¦L¥X 
    optab_node* ptr;
    int i, row = 0;
...
symlit_node * symlitNewNode(void) {	//·s¼W¤@µ§²Å¸¹©Î±`¼Æ¸ê®Æ 
...
void symtabCreate(void) {	//²Å¸¹ªíªì©l¤Æ 
    int i;
    for(i = 0; i < headerMax; i++)
    {
...
void littabCreate(void) {	//±`¼Æªíªì©l¤Æ 
    int i;
    for(i = 0; i < headerMax; i++)
    {
...
char *token(char temp[20]) {	//±N­ì©lµ{¦¡ªº¦r¦ê¥HªÅ¥Õ§@¤À³Î 
    char * tok;
    if(temp[0] == ' ') {
        return NULL;
    }
...
t_node * tNewNode(int start, int use) {	//·s¼W¤@µ§¤º¤å°O¿ý 
...
void tbuild(int loc, int length, int flag, int use) {	//«Ø¤º¤å°O¿ýªº¦ê¦C 
    if(thead == NULL) {
...
symlit_node * symlitFind(symlit_node * head, char tag[symtabNameMax]) {	//¦b²Å¸¹ªí©Î±`¼Æªí§ä´M¸ê®Æ¡A§ä¤£¨ì«h¦^¶ÇNULL 
...
void symlitInsert(symlit_node * head, char name[symtabNameMax], int loc) {	//±N¸ê®Æ´¡¤J²Å¸¹ªí 
    symlit_node * ptr;
    ptr = head;
    while(ptr->next != NULL) {
        ptr = ptr->next;
...
m_node * mNewNode(int loc) {	//·s¼W¤@µ§­×¥¿°O¿ý 
...
void mbuild(int loc) {	//«Ø­×¥¿°O¿ýªº¦ê¦C 
    if(mhead == NULL) {
...
void littabAddressing(FILE* fp, symlit_node * head) {	//¬°±`¼Æªíªº¸ê®Æ©w§} 
    symlit_node * ptr;
    if(head->next == NULL) {
        return;
    }
...
optab_node * optabFind(optab_node * head, char srcCode[srcCodeMax+1]) {	//¦b¾Þ§@½Xªí§ä´M¸ê®Æ¡A§ä¤£¨ì«h¦^¶ÇNULL 
    optab_node * ptr = head;
    while(ptr != NULL) {
...
void tModify(void) {	//­×¥¿¤º¤å°O¿ý  (±N°Ï¶ôªºªø«×¥[¤W¥h) 
    tptr = thead;
    while(tptr != NULL) {
...
void symlitPrint(symlit_node * head, int hash, int * row) {	//¦L¥X²Å¸¹ªí©Î±`¼Æªí 
    symlit_node * ptr;
    if(head->next != NULL) {
        ptr = head->next;
        while(ptr != NULL) {
...
void tPrint(FILE* fp, int address,int use, int length) {	//¦L¥X¤º¤å°O¿ý 
...
// ²Ä¤@½ü°j 
    ...
    FILE * fp_input = fopen("srcpro.txt", "r");			//2.9ªº¿é¤JÀÉ 
//    FILE * fp_input = fopen("srcpro2.11.txt", "r");	//2.11ªº¿é¤JÀÉ 
    ...
    while(fgets(srcStr, srcMax, fp_input) != NULL) {
        ...
        if(!strcmp(srcCode, "USE")) {	//°Ï¶ô 
            ...
            if(srcTag != NULL) {
                keyTemp = key(srcTag);
                if(symlitFind(symtabHeader[keyTemp], srcTag) == NULL) {	//¦b²Å¸¹ªí§ä¤£¨ì¸ê®Æ¡A©ó¬O·s¼W¨Ã´¡¤J 
            ...
            if(!strcmp(srcCode, "EQU")) {	//¥ý¿é¥X¦A¥[­p¼Æ¾¹ 
                fprintf(fp_output, "%04X %d %s \n", value, use, srcStr);
            }
            else{
                fprintf(fp_output, "%04X %d %s\n", blockTab[use].length, use, srcStr);
            ...
            if(!strcmp(srcCode, "WORD")) {	//WORD 
            ...
            else if(srcStr[srcExtendTagIndex] == '+') {	//®æ¦¡¥| ªø«×¬°4 
            ...
            else if(!strcmp(srcCode, "LTORG")) {	//LTORG §@±`¼Æªí©w§} 
                for(i = 0; i < headerMax; i++)
                {
                ...
                if(ptr != NULL) {
                    if(!strcmp(ptr->format, "1")) {	//®æ¦¡¤@ 
                        blockTab[use].length += 1;
                        tbuild(1, blockTab[use].length, 0, use);
                    }
                    else if(!strcmp(ptr->format, "2")) {	//®æ¦¡¤G 
            ...
            fprintf(fp_output, "%04X %d %s\n", blockTab[use].length, use, srcStr);
            for(i = 0; i < headerMax; i++) {	//END ±`¼Æªí©w§} 
                littabAddressing(fp_output, littabHeader[i]);
            }
        }
    }//while
    for(i = 0; i < blockTabMax-1; i++) {	//¦L¥X¼È¦s¾¹ªí 
        blockTab[i+1].start = blockTab[i].start + blockTab[i].length;
    }
    tModify();
    tptr = thead;	//«ü¦^¦ê¦C²Ä¤@­Ó 
...
//  ²Ä¤G½ü°j
    ...
    while(1) {
        ...
        srcOperand2 = token(temp3);
        if(!strcmp(srcCode, "START")) {	//START
            fprintf(fp_output, "H%-6s%06X%06X\nT%06X%02X", srcTag, blockTab[0].start, blockTab[blockTabMax-1].start + blockTab[blockTabMax-1].length, thead->start, thead->length);     //æ”¹
        ...
        if(strcmp(srcCode, "END") != 0) {	//not END 
            ...
            if(ptr != NULL) {
                ...
                if(!strcmp(ptr->format, "3/4")) {
                    if(srcStr[srcExtendTagIndex+1] == '+') {    //®æ¦¡¥| 
                        ...
                        if(srcOperand2 != NULL) {   //¯Á¤Þ x 
                            if(!strcmp(srcOperand2, "X")) {
                                ta += 8 * (int)pow(16,3);
                            }
                        }
                        if(srcStr[srcOperTagIndex + 1] == '#' && *srcOperand <= '9' && *srcOperand >= '0' ) {	//ª½±µ©w§} 
                            ta += atoi(srcOperand);
                        }
                        else if(srcStr[srcOperTagIndex+1] == '=') {	//±`¼Æ 
                            if(base == NULL) {
                                ta += 2 * (int)pow(16,3);	//p
                        ...
                        else if(strcmp(ptr->info, "null") != 0) {	//ºâ°ò©³¦ì²¾¥H¤Îµ{¦¡­p¼Æ¦ì²¾ 
                            if(base == NULL) {
                    ...
                    for(i = 0; i < regtabHeaderMax; i++) {	//¼È¦s¾¹1 
                        if(!strcmp(srcOperand, reg[i].name)) {
                            ta += reg[i].code * (int)pow(16,1);
                        }
                    }
                    if(srcOperand2 != NULL) {	//¼È¦s¾¹2 
            ...
            else if(!strcmp(srcCode, "BASE")) {	//BASE base«ü¼Ð«ü¦V¦¹¸ê®Æ 
                base = symlitFind(symtabHeader[key(srcOperand)], srcOperand);
            }
            else if(srcStr[srcExtendTagIndex + 1] == '=') {	//±`¼Æ 
                if(*srcCode == 'C') {
		...
		if(!strcmp(srcCode, "LTORG")) {	//LTORG ¤£¦L¥X¦ì¸m 
			printf("%2d         %s\n", row, srcStr);
		}else{
			printf("%2d  %04X %d %s\n", row, address, use, srcStr);
		}
